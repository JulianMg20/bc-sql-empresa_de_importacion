...
-- Une envíos (shipments) con sus proveedores (suppliers).
-- Solo muestra envíos que SÍ tienen un proveedor asociado
-- (en este esquema todo envío tiene proveedor por FK NOT NULL,
-- por lo que el INNER JOIN devuelve el total de la tabla principal).
-- ------------------------------------------------------------
SELECT
...
-- Encadena shipments + suppliers + products para un reporte
-- completo: qué proveedor envió qué producto y en qué categoría
-- ------------------------------------------------------------
SELECT
    sh.tracking_code   AS codigo_seguimiento,
    s.name              AS proveedor,
    ...
    sh.quantity         AS cantidad,
    sh.total_value      AS valor_total
FROM shipments AS sh
INNER JOIN suppliers AS s
    ON sh.supplier_id = s.supplier_id
...
-- ------------------------------------------------------------
-- CONSULTA 3 — LEFT JOIN: todos los proveedores, con o sin envíos
-- Muestra TODOS los proveedores (tabla padre), incluyendo los que
-- todavía no han enviado ninguna mercancía (registros hijos NULL)
-- ------------------------------------------------------------
SELECT
    s.name              AS proveedor,
    s.country           AS pais,
...
-- Proveedores que NUNCA han realizado un envío
-- (mismo LEFT JOIN de la consulta 3, filtrando los huérfanos)
-- ------------------------------------------------------------
SELECT
...
-- ------------------------------------------------------------
-- CONSULTA 4b — Detectar huérfanos del lado de customs
-- Envíos que NO tienen trámite de aduana registrado todavía
-- ------------------------------------------------------------
SELECT
    ...
    sh.total_value      AS valor_envio
FROM shipments AS sh
LEFT JOIN customs AS c
    ON sh.shipment_id = c.shipment_id
WHERE c.customs_id IS NULL
ORDER BY sh.ship_date;

-- ------------------------------------------------------------
-- CONSULTA 5 — Reporte agregado con JOINs
-- LEFT JOIN + GROUP BY + COUNT: cuántos envíos tiene cada
-- proveedor (incluyendo proveedores con CERO envíos)
-- ------------------------------------------------------------
SELECT
    s.supplier_id       AS id_proveedor,
    s.name              AS proveedor,
    ...
    COALESCE(SUM(sh.total_value), 0) AS valor_total_enviado
FROM suppliers AS s
LEFT JOIN shipments AS sh
    ON s.supplier_id = sh.supplier_id
